...
from pokeonta.models.scheduler import Scheduler
from pokeonta.tags import build_tag_set, fetch_tags
...
logger = get_logger(("pokeonta", "scheduler"))
...
def initialize_scheduler(loop=asyncio.get_event_loop()):
    ...
    for task in Scheduler.select():
        loop.create_task(_schedule(task, json.loads(task.payload)))
...
def schedule(
    ...
    **kwargs,
    ...
    task = _schedule_save(name, when, tags, json.dumps(payload))
...
class TaskScheduledForPast(Exception):
    pass


class TaskCallbackMissingSchedulerTag(Exception):
    pass
